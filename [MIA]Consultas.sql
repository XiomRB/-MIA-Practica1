...
--Mostrar el nombre del proveedor, número de teléfono, número de orden, total de la orden por la cual se haya pagado la mayor cantidad de dinero.
SELECT p.nombre AS PROVEEDOR, p.telefono AS TELEFONO, a.id AS ORDEN, SUM(da.subtotal) AS TOTAL 
...
----Mostrar el número de cliente, nombre, apellido y total del cliente que más productos ha comprado.
SELECT c.id AS ID, c.nombre AS NOMBRE, SUM(dv.cantidad) AS NO_PRODUCTOS, SUM(subtotal) AS TOTAL FROM DetalleVenta AS dv, Venta AS v, Cliente c
WHERE v.id = dv.venta AND c.id = v.cliente
GROUP BY v.cliente ORDER BY SUM(cantidad) DESC LIMIT 1;


----------------------------CONSULTA3------------------------------
----Mostrar la dirección, región, ciudad y código postal hacia la cual se han hecho más solicitudes de pedidos y a cuál menos (en una sola consulta).
...
----------------------------CONSULTA4-------------------------------
---Mostrar el número de cliente, nombre, apellido, el número de órdenes que ha realizado y el total de cada uno de los cinco clientes que más han comprado productos de la categoría ‘Cheese’.
SELECT v.cliente AS ID, cl.nombre AS CLIENTE, COUNT(v.id) AS NO_COMPRAS, SUM(dv.subtotal) AS TOTAL 
...
----los clientes que más han comprado y los que menos han comprado (en dinero) utilizando una sola consulta.
...
--------------CONSULTA6-------------------
--Mostrar el nombre de la categoría más y menos vendida y el total vendido en dinero (en una sola consulta).
...
-------------------------CONSULTA7----------------------
--. Mostrar el top 5 de proveedores que más productos han vendido (en dinero) de la categoría de productos ‘Fresh Vegetables’.
...
------------------------------CONSULTA8----------------------------
-------Mostrar la dirección, región, ciudad y código postal de los clientes que más han comprado y de los que menos (en dinero) en una sola consulta.
...
SELECT c.nombre AS CLIENTE, SUM(dv.cantidad) AS NO_PRODUCTOS FROM Cliente AS c, DetalleVenta AS dv, Venta AS v, Producto AS p, Categoria AS cat
